...
export function getTopIPv4(g: GlobalJson): V4TopItem[] { return g.top.ipv4; }
export function getTopIPv6(g: GlobalJson): V6TopItem[] { return g.top.ipv6; }
...
export function getAsnsByCountry(g: GlobalJson, cc: string): Array<V4TopItem | V6TopItem> {
  ...
  const seen = new Set<number>(); const out: Array<V4TopItem | V6TopItem> = [];
  for (const r of merged) { if (!seen.has(r.asn)) { seen.add(r.asn); out.push(r); } }
  return out.sort((a,b)=> (b as any).v4_slash24s - (a as any).v4_slash24s || (b as any).v6_slots - (a as any).v6_slots);
}

export function getAsnsByOrg(g: GlobalJson, org: string): Array<V4TopItem | V6TopItem> {
  const key = org.trim().toLowerCase();
  const merged = [...g.top.ipv4, ...g.top.ipv6]
    .filter(r => r.org.toLowerCase() === key || r.name.toLowerCase() === key);
  const seen = new Set<number>(); const out: Array<V4TopItem | V6TopItem> = [];
  for (const r of merged) { if (!seen.has(r.asn)) { seen.add(r.asn); out.push(r); } }
  return out.sort((a,b)=> (b as any).v4_slash24s - (a as any).v4_slash24s || (b as any).v6_slots - (a as any).v6_slots);
}
...
const ISO_COUNTRIES: Record<string,string> = {
  ...
  KZ:"Kazakhstan", KE:"Kenya", KI:"Kiribati", KP:"Korea (DPRK)", KR:"Korea (Republic of)", KW:"Kuwait", KG:"Kyrgyzstan",
  LA:"Lao PDR", LV:"Latvia", LB:"Lebanon", LS:"Lesotho", LR:"Liberia", LY:"Libya", LI:"Liechtenstein", LT:"Lithuania", LU:"Luxembourg",
  ...
  UG:"Uganda", UA:"Ukraine", AE:"United Arab Emirates", GB:"United Kingdom", US:"United States of America", UM:"US Minor Outlying Islands", UY:"Uruguay", UZ:"Uzbekistan",
