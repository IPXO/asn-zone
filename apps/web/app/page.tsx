    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-4 p-4 text-sm">
          <div><div className="text-gray-500">Total ASNs</div><div className="font-mono">{fmt(data.stats?.asns_total)}</div></div>
          <div><div className="text-gray-500">IPv4 announced</div><div className="font-mono">{fmt(data.stats?.ipv4_announced)}</div></div>
          <div><div className="text-gray-500">IPv6 announced</div><div className="font-mono">{fmt(data.stats?.ipv6_announced)}</div></div>
        </div>
      </section>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <section className="rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
            ...
            <a href="/top/ipv4" className="text-sm text-indigo-600 hover:underline">View all</a>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50/60 dark:bg-white/5">
              <tr className="[&>th]:px-3 [&>th]:py-2 [&>th]:text-left [&>th]:font-medium text-gray-600 dark:text-gray-300">
                <th>Rank</th><th>ASN</th><th>Org</th><th className="text-right">IPv4 (/24 eq)</th>
              </tr>
            </thead>
            <tbody className="[&>tr>td]:px-3 [&>tr>td]:py-2 [&>tr>td]:border-b [&>tr>td]:border-gray-100 dark:[&>tr>td]:border-white/5">
              {t4.map((r, i) => (
                <tr key={r.asn}>
                  <td>{i+1}</td>
                  <td className="font-mono"><a className="text-indigo-600 hover:underline" href={`/asn/${r.asn}`}>AS{r.asn}</a></td>
                  <td>{r.org ?? r.name ?? "—"}</td>
                  <td className="text-right font-mono">{fmt(r.v4_slash24s)}</td>
                </tr>
              ))}
              {!t4.length && <tr><td colSpan={4} className="text-center text-gray-500 py-6">No data yet</td></tr>}
            </tbody>
          </table>
        </section>
        ...
        <section className="rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
            ...
            <a href="/top/ipv6" className="text-sm text-indigo-600 hover:underline">View all</a>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50/60 dark:bg-white/5">
              <tr className="[&>th]:px-3 [&>th]:py-2 [&>th]:text-left [&>th]:font-medium text-gray-600 dark:text-gray-300">
                <th>Rank</th><th>ASN</th><th>Org</th><th className="text-right">/32 slots</th>
              </tr>
            </thead>
            <tbody className="[&>tr>td]:px-3 [&>tr>td]:py-2 [&>tr>td]:border-b [&>tr>td]:border-gray-100 dark:[&>tr>td]:border-white/5">
              {t6.map((r, i) => (
              ...
              {!t6.length && <tr><td colSpan={4} className="text-center text-gray-500 py-6">No data yet</td></tr>}
            </tbody>
          </table>
        </section>
      </div>
